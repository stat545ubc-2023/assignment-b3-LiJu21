...
ui <- fluidPage(
  ...
  # Include image
  img(src = "bcliquor.svg", height = "100px", width = "auto"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("alcoholInput", "Alcohol Content", 0, 20, c(4, 10), step = 0.1, post = "%"),
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    ggplot(prices(), aes(Alcohol_Content, fill = Type)) +
      ...
      labs(title = "Count of Products by Alcohol Content",
           x = "Alcohol Content",
           y = "Count") +
      theme_classic(20)
  })
  ...
  output$scatterPlot <- renderPlot({
    ...
    filtered_prices <- prices() %>%
      filter(!is.na(Sweetness), !is.na(Alcohol_Content))

    # Scatter plot
    ggplot(filtered_prices, aes(Sweetness, Alcohol_Content, color = Type)) +
      geom_point() +
      labs(title = "Sweetness vs Alcohol Content",
           x = "Sweetness",
           y = "Alcohol Content") +
      theme_classic(20)
  })
